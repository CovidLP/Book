...
### Next contents
#
# (1) Generate Figure 5.6 from the "rds" files available in the Github path "Part2/Chapter6/data/"
# (2) Download US data from John Hopkins repository
# (3) Run generalised logistic model with 2 curves using Stan and generate the "lt_predict" object used in Figure 5.6
#
# The codes in (2) and (3) generate "rds" files with objects to generate Figure 5.6. These "rds" files
# are reduced when compared to the ones in "Part2/Chapter5/data/", but have enough information to
# generate Figure 5.6 with the codes in (1). 



### (1) Generate Figure 5.6

# "rds" file
data_rds <- paste0("C:/_covid_T5/",t)
data_US <- readRDS(paste0(data_rds,"/US_n2_gen.rds"))
# data_rds <- paste0("Part2/Chapter5/data/")
# data_BR <- readRDS(paste0(data_rds,"US_n2_gen.rds"))
...
// curve: Generalized Logistic (2 curves)
...
parameters {
  real b1_1;
  real<lower=0> f1;
  real<lower=0, upper=pop*perPop*exp((f1+1)*b1_1)> a1;
  real b1_2;
  real<lower=0> f2;
  real<lower=0, upper=pop*perPop*exp((f2+1)*b1_2)> a2;
  real<lower=0> c1;
  real<lower=0> c2;
  real eta;
  real<lower=0> sigma;
...
transformed parameters{
  ...
  for(t in 1:n){
    mu1[t] = exp(log((f1+1))+log(a1)+log(c1)-(c1*(t))-(f1+2)*log( b1+exp(-c1*(t)) ));
    mu2[t] = exp(log((f2+1))+log(a2)+log(c2)-(c2*(t))-(f2+2)*log( b2+exp(-c2*(t)) ));
    mu[t] = (1-normal_cdf(t,eta,sigma)) * mu1[t] + normal_cdf(t,eta,sigma) * mu2[t];
  }
}
...
model {
   ...
   c1 ~ gamma(2,9);
   c2 ~ gamma(2,9);
   f1 ~ inv_gamma(0.01,0.01);   // shape, scale
   f2 ~ inv_gamma(0.01,0.01);   // shape, scale
   eta ~ normal(1,100);
...
# Stan configurations
params = c("a1","b1","c1","f1","a2","b2","c2","f2","mu","eta","sigma")
burn_in = 1e4
lag = 5
sample_size = 1e3
...
# "predL2_gen" function: generate a sample from the predictive distribution
predL2_gen <- function(L=100,B,pop,casos,a1,b1,c1,f1,a2,b2,c2,f2,eta,sigma){
  M <- length(a1)
  Max <- 2*pop
  y.fut <- mu <- mu1 <- mu2 <- matrix(-Inf, ncol=L,nrow=M)
  
  for(i in 1:L){
    mu1[,i] <- exp(log((f1+1))+log(a1)+log(c1)-(c1*(B+i))-(f1+2)*log( b1+exp(-c1*(B+i)) ));
    mu2[,i] <- exp(log((f2+1))+log(a2)+log(c2)-(c2*(B+i))-(f2+2)*log( b2+exp(-c2*(B+i)) ));
    mu[,i] <-  (1-pnorm(B+i,eta,sigma)) * mu1[,i] + pnorm(B+i,eta,sigma) * mu2[,i];
...
fut <- predL2_gen(L=L,t,pop*pp,Y[[i-2]][t],
                  c(mod_chain[[a1_pos]]),c(mod_chain[[b1_pos]]),c(mod_chain[[c1_pos]]),c(mod_chain[[f1_pos]]),
                  c(mod_chain[[a2_pos]]),c(mod_chain[[b2_pos]]),c(mod_chain[[c2_pos]]),c(mod_chain[[f2_pos]]),
                  c(mod_chain[[eta_pos]]),c(mod_chain[[sigma_pos]]))
