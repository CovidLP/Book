...
### Next contents
#
# (1) Generate Figure 5.7 from the "rds" files available in the Github path "Part2/Chapter5/data/"
# (2) Download US data from John Hopkins repository
# (3) Run generalised logistic model with 2 curves using Stan and generate the "lt_predict" object used in Figure 5.7
#
# The codes in (2) and (3) generate "rds" files with objects to generate Figure 5.7. These "rds" files
# are reduced when compared to the ones in "Part2/Chapter5/data/", but have enough information to
# generate Figure 5.7 with the codes in (1). 



### (1) Generate Figure 5.7
...
# Stan configurations
params = c("a1","b1","c1","alpha1","a2","b2","c2","alpha2","mu","delta1","delta2")
burn_in = 1e4
lag = 5
sample_size = 1e3
...
# "predL2_azzalini" function: generate a sample from the predictive distribution
predL2_azzalini <- function(L=100,B,pop,casos,a1,b1,c1,alpha1,a2,b2,c2,alpha2,delta1,delta2){
  M <- length(a1)
  Max <- 2*pop
...
fut <- predL2_azzalini(L=L,t,pop*pp,Y[[i-2]][t],
                       c(mod_chain[[a1_pos]]),c(mod_chain[[b1_pos]]),c(mod_chain[[c1_pos]]),c(mod_chain[[alpha1_pos]]),
                       c(mod_chain[[a2_pos]]),c(mod_chain[[b2_pos]]),c(mod_chain[[c2_pos]]),c(mod_chain[[alpha2_pos]]),
                       c(mod_chain[[delta1_pos]]),c(mod_chain[[delta2_pos]]))
