...
#### Figure 6.1 ####
SAf_6.1 = load_covid("South Africa",last_date = "2020-09-01")
# SAf_6.1 = readRDS("data/601_SAf.rds") # Run this to load the data from a file

SAf_est = pandemic_model(SAf_6.1,covidLPconfig = TRUE)
# SAf_est = readRDS("data/601_SAf.rds") # Run this to load the data from a file
...
times = 1:nrow(SAf_6.1$data)
contoursAB = rep(0,100*100)
contoursAC = rep(0,100*100)
contoursBC = rep(0,100*100)
for (i in 1:100)
  for (j in 1:100){
    contoursAB[(i-1)*100 + j] = sum(dpois(SAf_6.1$data$new_cases,muFun(times,as[i],bs[j],pars[3]),log=TRUE))
    contoursAC[(i-1)*100 + j] = sum(dpois(SAf_6.1$data$new_cases,muFun(times,as[i],pars[2],cs[j]),log=TRUE))
    contoursBC[(i-1)*100 + j] = sum(dpois(SAf_6.1$data$new_cases,muFun(times,pars[1],bs[i],cs[j]),log=TRUE))
  }
